...
- React components use PascalCase file names such as `components/LookbookGenerator.tsx`.
- Context providers also use PascalCase with `Context` in the file name, such as `contexts/ApiProviderContext.tsx`.
- Hooks use `use` prefix and camelCase file names, such as `hooks/useClothingTransfer.ts`.
- Services use camelCase file names, such as `services/imageEditingService.ts`.
...
Use `components/LookbookGenerator.tsx`, `hooks/useLookbookGenerator.ts`, and `utils/lookbookPromptBuilder.ts` as the baseline example.

## Component Style

- Components are written as `React.FC`.
- Top-level screens often keep helper callbacks inline with `useCallback`.
- Lazy-loaded screens are declared in `App.tsx`.
...
- User-facing errors should pass through `getErrorMessage(err, t)` from `utils/imageUtils.ts`.
...
- UI strings come from `locales/en.ts` and `locales/vi.ts`.
...
- Hooks and components build small config objects and pass them to `services/imageEditingService.ts`.
...
- Adding more feature logic directly into large view components like `components/VirtualTryOn.tsx`.
- Duplicating provider-routing logic outside `services/imageEditingService.ts`.
