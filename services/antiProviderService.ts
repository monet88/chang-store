...
export interface AntiProviderConfig {
  baseUrl: string;
  apiKey?: string | null;
}
...
function normalizeError(error: unknown): Error {
  ...
  return new Error('error.api.antiProviderFailed');
}
...
export async function generateTextAnti(
  prompt: string,
  model: string,
  config: AntiProviderConfig
): Promise<string> {
  try {
    if (!config.baseUrl) {
      throw new Error('error.api.antiProviderFailed');
...
export async function generateTextFromImageAnti(
  image: ImageFile,
  prompt: string,
  model: string,
  config: AntiProviderConfig
...
export async function generateImageAnti(
  prompt: string,
  model: string,
  config: AntiProviderConfig,
...
export async function editImageAnti(
  image: ImageFile,
  prompt: string,
  model: string,
  config: AntiProviderConfig,
...
export interface ImageChatSessionAnti {
  sendRefinement(prompt: string, currentImage: ImageFile): Promise<ImageFile>;
  getHistory(): RefinementHistoryItemAnti[];
  reset(): void;
}

export interface RefinementHistoryItemAnti {
  prompt: string;
  timestamp: number;
}
...
/**
 * Create a chat session for iterative image refinement using Anti provider.
 * Maintains conversation history client-side.
 */
export function createImageChatSessionAnti(
  model: string,
  config: AntiProviderConfig
): ImageChatSessionAnti {
  let history: RefinementHistoryItemAnti[] = [];
