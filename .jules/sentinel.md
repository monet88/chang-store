...
## 2026-06-12 - ReDoS and Memory DoS in Data URL Parsing (Update)
**Vulnerability:** Gateway image endpoints parsed massive base64 image data URLs calling `.toLowerCase()` on the entire string (`value.toLowerCase().indexOf(...)`). This allocates huge amounts of memory and CPU for 10MB+ payload strings, creating a severe memory Denial of Service (DoS) attack vector.
**Learning:** Using `.toLowerCase()` on multi-megabyte strings to check a prefix or substring creates massive duplicate strings in memory, resulting in severe garbage collection pauses and event loop blocking.
**Prevention:** For large payloads, always use bounded string operations (`value.substring(0, 5).toLowerCase()`) or use explicit boundary indexes (`indexOf(",", 5)`) to isolate and validate only the small metadata segment, avoiding full-string duplication.
