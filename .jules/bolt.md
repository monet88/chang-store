...
## 2026-06-22 - Prevent massive, cascading re-renders via useMemo for React Context Providers
**Learning:** Returning inline object literals (e.g., `{{ showToast }}`) as the `value` in a highly utilized React Context Provider forces every consumer across the application to re-render whenever the provider's state is modified, causing unnecessary performance overhead.
**Action:** Always wrap the `value` object passed to React Context Providers with `useMemo` to preserve object identity across re-renders, mitigating massive cascading re-renders across the component tree.
