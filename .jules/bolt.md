...
## 2025-02-14 - Prevent Provider Cascades via useMemo
**Learning:** Passing inline objects directly as the `value` prop to React Context Providers (e.g., `<MyContext.Provider value={{ a, b }}>`) breaks object identity on every single render of the provider. This causes all consuming components to re-render, even if the underlying values (`a` and `b`) haven't changed.
**Action:** Always wrap the object passed to a Context Provider's `value` prop in a `useMemo` hook, with its dependencies explicitly listed (e.g., `useMemo(() => ({ a, b }), [a, b])`). Make sure the dependencies themselves (especially functions) are wrapped in `useCallback`.
