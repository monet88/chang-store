...
## 2025-02-14 - Optimize Translation String Lookups and Interpolations
**Learning:** Frequent translation operations (`t(...)`) using string array allocations via `.split()` and `.reduce()` for object lookups, as well as instantiating dynamic `new RegExp` objects in loops for interpolations, introduce unnecessary CPU overhead and increase Garbage Collection pauses.
