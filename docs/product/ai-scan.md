...
hemline details) before synthesis. The blueprint is model-agnostic observed
evidence; the active model-family prompt policy decides how to express it as a
subordinate specification. The layer exists so fabric, construction and drape
survive a single-pass image model that would otherwise flatten them.
...
3. At generation time the feature awaits the same source-set analysis (served
   from cache when the panel already ran it) and passes the model-agnostic
   blueprint to the active model-family prompt policy. A failed, cancelled or
   disabled analysis resolves to `null`, and the feature ships its base prompt —
   generation is never blocked by AI Scan.
...
| --- | --- | --- |
| Virtual Try-On (multi-model) | one analysis per subject: the target garments, then that subject's photo | active model-family prompt |
| Virtual Try-On (wardrobe) | one analysis per set: that set's garments, then the subject | active model-family prompt |
| Lookbook | clothing images plus the fabric texture image (one slot reserved for the swatch); variations and close-ups reuse the analysis of the main they derive from | active main, variation and close-up prompt policies |
| Identity Transfer | one analysis per destination photo | active model-family destination rules |
...
The E-Com Pack (Clothing Transfer) runs the same model-agnostic analysis through
its own always-on lane. Its active Gemini or GPT Image prompt policy owns how the
resulting blueprint is expressed during synthesis.
