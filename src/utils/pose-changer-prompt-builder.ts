...
export const buildTextPosePrompt = (promptText: string, framingInstruction: string): string => `
  ...
  **CRITICAL RULES**:
  ...
  2.  **Preserve Identity**: The model's identity (face, hair, body shape), their entire outfit (design, color, texture), and the entire background from the Source Image must be faithfully preserved.
...
export const buildReferencePosePrompt = (customPosePrompt: string, framingInstruction: string): string => `
  ...
  **CRITICAL RULES**:
  ...
  2.  **Preserve Subject, Clothing, and Background**: The model's identity, their entire outfit, and the entire background from the 'Subject Image' must be faithfully preserved.
