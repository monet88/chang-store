/**
 * Gemini Identity Transfer prompt policy.
 * Owns Gemini-specific interleaved role/image parts framing, reference roles,
 * and the complete authority brief for Identity Transfer.
 */
...
/**
 * Build interleaved Part[] for a Gemini Identity Transfer job.
 * Destination image first, then Face Reference, optional Body Reference, and the full task text.
 */
export const buildGeminiIdentityTransferParts = (
  ...
  parts.push({ text: buildGeminiTaskText(input) });
  return parts;
};

const buildGeminiTaskText = (input: IdentityTransferPromptInput): string => {
...
## FINAL INVARIANTS
One destination produces one edited image. Preserve destination pose, skeleton placement, spatial performance, and camera relationships, plus composition, lighting, and all unrelated details. The worn makeup look — lashes, brows, eye and lip styling, lip colour and finish, blush, contour — is taken from the Face Reference, re-lit by the destination lighting and laid thinly over real skin; nails, outfit and the scene stay with the Destination Image. The destination expression and colour grade win over the Face Reference's own expression, lighting and rendering: the transferred identity is lit, graded, and performing exactly as the Destination Image. ${finalBodyRule} Face Reference controls stable facial identity, the underlying skin tone family (re-rendered in the destination grade), identity-specific marks, hair, and the worn makeup look. A multi-panel Face Reference supplies one single identity and never its panel layout, and no text, label, or watermark from any reference may appear in the result. Body Reference, when present, controls morphology and silhouette, replacing destination body proportions. Destination pose and posture always win. Avoid plastic or waxy skin, poreless porcelain finish, airbrushed beauty-filter smoothing, smeared foundation, painted-on hair, and dead eyes; keep pores, fine lines and small blemishes visible.`;
