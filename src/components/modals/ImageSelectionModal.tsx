    ...
    return (
        ...
        >
            <div className="flex justify-between items-center p-4 text-white w-full max-w-7xl mx-auto flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                <h2 id="image-selection-modal-title" className="text-xl md:text-2xl font-bold">{t('imageSelectionModal.title')} ({images.length})</h2>
                <button onClick={onClose} className="p-2 rounded-full bg-black/30 hover:bg-black/50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" aria-label={t('gallery.closeAria')}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 max-w-7xl mx-auto">
                        {images.map((image, index) => (
                            <button
                                ...
                                className="relative aspect-square bg-zinc-800 rounded-lg overflow-hidden group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-900 focus-visible:ring-amber-500 transition-transform transform hover:scale-105"
                                aria-label={`${t('imageSelectionModal.select')} ${t('gallery.altText', { index: index + 1 })}`}
                            >
                                <img src={`data:${image.mimeType};base64,${image.base64}`} alt={t('gallery.altText', { index: index + 1 })} loading="lazy" decoding="async" className="w-full h-full object-cover" />
