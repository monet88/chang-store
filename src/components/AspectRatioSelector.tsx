...
const AspectRatioSelector: React.FC<AspectRatioSelectorProps> = React.memo(({ aspectRatio, setAspectRatio }) => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2">
      <span id="aspect-ratio-label" className="font-medium text-zinc-300">{t('studio.workflows.aspectRatioLabel')}</span>
      <div
        role="group"
      ...
      >
        {RATIOS.map(ratio => (
          <button
            ...
            className={`flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 ${aspectRatio === ratio ? 'bg-white text-zinc-950' : 'text-zinc-300 hover:bg-white/6'
              }`}
          >
            {ratio}
          </button>
