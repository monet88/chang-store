                  ...
                  {wardrobe.results.map((resultSet, setIdx) => (
                    <div key={resultSet.setId} className="space-y-3">
                      ...
                      {(resultSet.status === 'processing' || resultSet.status === 'pending') && (
                        <div className="flex aspect-[3/4] max-h-48 items-center justify-center rounded-xl border border-white/10 bg-black/30 animate-skeleton-shimmer transition-opacity duration-300">
                          <Spinner className="h-8 w-8 border-white" />
                        </div>
                      )}
