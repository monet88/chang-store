...
const GptVirtualTryOn: React.FC = () => {
  const {
    mode,
    setMode,
    isAnyGenerating,
                      ...
                      : item.status === 'processing' || item.status === 'pending'
                        ? [
                          <div
                            ...
                            className={`flex aspect-[3/4] flex-col items-center justify-center gap-3 rounded-[24px] border border-white/10 bg-black/30 p-4 transition-opacity duration-300 ${item.status === 'processing' ? 'animate-skeleton-shimmer' : ''
                              }`}
                          >
                  ...
                  {wardrobe.results.map((resultSet, setIdx) => (
                    <div key={resultSet.setId} className="space-y-3">
                      ...
                      {(resultSet.status === 'processing' || resultSet.status === 'pending') && (
                        <div className="flex aspect-[3/4] max-h-48 items-center justify-center rounded-[24px] border border-white/10 bg-black/30 animate-skeleton-shimmer transition-opacity duration-300">
                          <Spinner className="h-8 w-8 border-white" />
                        </div>
                      )}
...
export default GptVirtualTryOn;
