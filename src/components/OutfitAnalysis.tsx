                  ...
                  <div className="flex flex-wrap gap-2 p-2 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
                    {selectedPresets.map((preset) => (
                      <div key={preset} className="bg-zinc-700 text-zinc-200 text-xs font-semibold pl-3 pr-2 py-1.5 rounded-full flex items-center gap-2">
                        <span>{PRESETS.find((presetItem) => presetItem.key === preset)?.label}</span>
                        <button onClick={() => handleTogglePreset(preset)} className="text-zinc-400 hover:text-white bg-zinc-600/50 hover:bg-zinc-500/50 rounded-full p-0.5" aria-label={t('outfitAnalysis.removeStyleAria', { style: PRESETS.find((presetItem) => presetItem.key === preset)?.label })}>
            ...
            {redesignResults.map((result) => (
              <div key={result.preset}>
                <h3 className="text-xl font-semibold text-center mb-4 text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400">
                  {t('outfitAnalysis.redesignSuggestionPreset', { preset: PRESETS.find((presetItem) => presetItem.key === result.preset)?.label || '' })}
                </h3>
                <div className={`grid gap-4 ${result.images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                  {result.images.map((image, index) => {
                    const presetLabel = PRESETS.find((presetItem) => presetItem.key === result.preset)?.label || result.preset;
