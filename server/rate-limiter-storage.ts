...
export class PostgresRateLimitStorage implements RateLimitStorage {
  ...
  async increment(key: string, windowStart: number): Promise<number> {
    ...
    if (this.lastCleanupWindowStart === null || windowStart > this.lastCleanupWindowStart) {
      await this.cleanExpiredWindows(windowStartDate);
      this.lastCleanupWindowStart = windowStart;
    }
  ...
  private async cleanExpiredWindows(windowStartDate: string): Promise<void> {
    await this.db.query(
